...
public class RemoveRoleCommandHandler extends AdminCommandHandler {
    ...
    public void handleCommand(@NotNull CommandModel model) {
        ...
        roleController.removeRole(model.getOriginGuild(), command[1]);

        sendReply(model.getOriginalMessage(), "```Removed role " + command[1] + "```");
    }
}
